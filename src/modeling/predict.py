...
def forecast_future(model_payload: dict, recent_data: pd.DataFrame, days_ahead: int = 3) -> pd.DataFrame:
    model = model_payload["model"]
    features = model_payload["features"]

    #tomar el promedio de los ultimos 5 dias como base para "mañana"
    #esto es una simulacion simple basada en persistencia + ruido :"v
    window_size = 5
    recent_mean = recent_data[features].tail(window_size).mean()

    #generar features futuras con ruido aleatorio
    future_features = []
    for _ in range(days_ahead):
        #ruido normal (0, 5%) sobre la media reciente
